...
class train_ticket_choose_UI:
    ...
    def get_ticket_thread(self):
        self.ticket_choose_train_thread = threading.Thread(
            target=get_ticket, args=(self.text_train_code, self.period_start_name, self.period_end_name, self.train_go_date,
                                     self.condition, self.choose_train_ticket), name="thread10", daemon=True)
        ...
        if self.condition=="1":
            ...
            for i in range(len(self.train_code_list)):
                if self.text_train_code == self.train_code_list[i]:
                    ...
                    if (self.train_ticket_condition[i]=="票已售罄，无法购票" or self.train_ticket_condition[i]=="该票未起售"):
                        self.count_1+=1
                    else:
